/**
 * @file <argos3/plugins/robots/crazyflie/simulator/crazyflie_camera_equipped_entity.cpp>
 *
...
namespace argos {
   ...
   CCrazyflieCameraEquippedEntity::CCrazyflieCameraEquippedEntity(CComposableEntity* pc_parent) :
      CEntity(pc_parent),
      m_fFocalLength(0.0f),
      m_fRange(0.0f),
      m_nImagePxWidth(0),
   ...
   void CCrazyflieCameraEquippedEntity::Init(TConfigurationNode& t_tree) {
      try {
         CEntity::Init(t_tree);
   ...
   void CCrazyflieCameraEquippedEntity::Enable() {
      if (m_psAnchor) {
         m_psAnchor->Enable();
      }
   }
   ...
   void CCrazyflieCameraEquippedEntity::Disable() {
      if (m_psAnchor) {
         m_psAnchor->Disable();
      }
   }
   ...
   REGISTER_STANDARD_SPACE_OPERATIONS_ON_ENTITY(CCrazyflieCameraEquippedEntity);
